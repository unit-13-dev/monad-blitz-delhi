...
export default function MarketDetailPage() {
  ...
  const marketId = params.marketId ? parseInt(params.marketId as string) : null
  ...
  const fetchMarket = useCallback(async () => {
    if (!readOnlyContract || marketId === null) return
    ...
    try {
      ...
      const questionText = market.question || status.question || `Market #${marketId}`
      setQuestion(questionText)

      if (address) {
        try {
          const bet = await readOnlyContract.getUserBet(marketId, address)
          setUserBet(bet)
        } catch (err) {
  ...
  const fetchRecentBets = useCallback(async () => {
    if (!marketId || !readOnlyContract || marketId === null) return

    try {
      console.log(`📊 Fetching recent bets for market ${marketId}...`)
      const contractInstance = readOnlyContract.getContract()
      const filter = contractInstance.filters.BetPlaced(marketId, null)
      
      let events: ethers.Event[] = []
      try {
        const queriedEvents = await contractInstance.queryFilter(filter, 0)
        events = queriedEvents
        console.log(`✅ Found ${events.length} BetPlaced events for market ${marketId}`)
      } catch (eventError: any) {
        console.warn("⚠️ Could not query events (may not be indexed yet):", eventError?.message || eventError)
        events = []
      }
      ...
      if (events.length > 0) {
        const betsWithTimestamps: Array<RecentBet & { blockNumber: number; blockTime?: number }> = []
        
        for (const event of events) {
          try {
            const participantAddress = event.args?.user
            const prediction = event.args?.prediction
            const amount = event.args?.amount
            
            if (!participantAddress) {
              console.warn("Event missing user address:", event)
              continue
            }
            
            const betData: RecentBet & { blockNumber: number; blockTime?: number } = {
              username: `${participantAddress.slice(0, 6)}...${participantAddress.slice(-4)}`,
            ...
            try {
              if (event.blockNumber) {
                const block = await contractInstance.provider.getBlock(event.blockNumber)
                if (block) {
                  betData.blockTime = block.timestamp
                  betData.timestamp = new Date(block.timestamp * 1000).toISOString()
                }
              }
            } catch (blockError) {
            ...
            betsWithTimestamps.push(betData)
          } catch (err) {
        ...
        const betsToProcess = betsWithTimestamps.slice(0, 50)
        
        const usernamePromises = betsToProcess.map(async (bet) => {
          try {
            const userResponse = await fetch(`/api/users?walletAddress=${bet.address}`)
            if (userResponse.ok) {
              const userData = await userResponse.json()
              if (userData.username) {
                bet.username = userData.username
              }
            }
          } catch (err) {
            console.warn(`Failed to fetch username for ${bet.address}:`, err)
          }
          return bet
        })
        
        const betsWithUsernames = await Promise.all(usernamePromises)
        
        betsData.push(...betsWithUsernames.map(({ blockNumber, blockTime, ...bet }) => bet))
      }
      
      if (betsData.length === 0) {
        console.log("⚠️ No bets from events, fetching bets directly from contract...")
        
        try {
          const market = await readOnlyContract.getMarket(marketId)
          
          if (market.participantCount > 0) {
            console.log(`Market has ${market.participantCount} participants, fetching their bets...`)
            
            const allParticipants = await readOnlyContract.getAllParticipants()
            console.log(`Found ${allParticipants.length} total participants across all markets`)
            
            const marketBetsPromises = allParticipants.slice(0, 100).map(async (address): Promise<RecentBet | null> => {
              try {
                const bet = await readOnlyContract.getUserBet(marketId, address)
            ...
            const allBets = await Promise.all(marketBetsPromises)
            const validBets = allBets.filter((bet): bet is RecentBet => bet !== null)
            
            console.log(`✅ Found ${validBets.length} bets from contract for market ${marketId}`)
            betsData.push(...validBets)
          } else {
            console.log(`ℹ️ Market ${marketId} has no participants yet`)
          }
        } catch (participantError) {
          console.error("❌ Error fetching bets from participants:", participantError)
        }
      }

      if (betsData.length > 0) {
        console.log(`✅ Total loaded: ${betsData.length} recent bets`)
        setRecentBets(betsData.slice(0, 50))
      } else {
        console.log("ℹ️ No bets found for this market")
        setRecentBets([])
      }
    } catch (err) {
  ...
  if (marketNotFound || !marketData || marketId === null) {
    return (
      <AppLayout>
        <section className="bg-white min-h-screen py-12">
          <div className="mx-auto max-w-7xl px-4">
            <Card className="border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-12 bg-yellow-400">
              <div className="text-center">
                <h1 className="text-3xl font-black uppercase mb-4">MARKET NOT FOUND</h1>
                <p className="text-lg font-bold">The market you're looking for doesn't exist</p>
                <Link href="/live-event">
                  <Button className="mt-6 bg-black text-white border-4 border-black hover:bg-white hover:text-black font-black uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
  ...
  return (
    <AppLayout>
      <section className="bg-white min-h-screen py-12">
        <div className="mx-auto max-w-7xl px-4">
          <Link href="/live-event">
            <Button className="mb-6 bg-black text-white border-4 border-black hover:bg-white hover:text-black font-black uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
              <ArrowLeft className="h-4 w-4 mr-2" />
              BACK TO MARKETS
            </Button>
          </Link>
